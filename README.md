...
9. 작업화면
    ...
    https://github.com/user-attachments/assets/99b9e22e-85ac-4ab9-ad3b-b01c50bfa776
...
1. 뉴스 게시글 수정
    ...
    - public string Writer -> `pulbic string? Writer`로 변경!!
...
5. EntityFramework로 자동생성된 테이블 컬럼타입변경
...
6. 페이징
    ...
    - NewController Index() 메서드 내 완전 수정!
...
- 웹사이트만 새로 생성
    ...
    2. wwwroot/index.html 파일 생성 - VS Code의 Live Server 예제와 동일(정적페이지)
    ...
    6. Views 폴더 하위 파일들이 필요
        - _ViewImports.cshtml
        - _ViewStart.cshtml
        - _Shared/_Layout.cshtml.css
    7. index.html에 화면UI 구현
    8. AJAX로 WebAPI 호출하고 데이터를 화면에 뿌리는 로직 추가
    9. input 태그로 검색부분 구현
        - Javascript와 데이터를 주고받으면 form 태그가 없어도 됨
...
1. 인스턴스 진입
    ...
    5. IIS 서비스
        ...
        - Add FtpSite...
            - 이름, 물리적 경로 선택
            - IP/All : All Unassigned, Port/21, SSL/Allow SSL 선택
...
9. Controller/NewsController.cs
    - Eidt() Post메서드에 파일 파라미터 추가
    - 파일저장 로직 처리(Create() 메서드 로직 복사)
10. VS IIS Express 사용시
    - 최대 30MB 제약이 걸려있음 - ERR_CONNECTION_RESET 발생
    - 업로드 크기 제한을 두더라도 최대사이즈는 설정필요
11. Program.cs
    - 최대 업로드 크기 설정
...
- 자세한 내용은 [이미지폴더](./image/Portfolio/) 확인

<img src="./image/Portfolio/MyPortfolio_003.png" width="600">

<img src="./image/Portfolio/MyPortfolio_002.png" width="600">

<img src="./image/Portfolio/MyPortfolio_017.png" width="600">