...
namespace MyPortfolioWebApp.Controllers
{
    public class NewsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int page = 1, string search = "")
        {
            ...
            // 최종단계
            // 페이지 개수
            //var totalCount = _context.News.FromSql($@"SELECT Id FROM News WHERE Title LIKE '%{search}%'").Count(); // 현재 문제발생!!
        ...
        public async Task<IActionResult> Create([Bind("Id,Title,Description")] News news, IFormFile? UploadFile)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (UploadFile != null && UploadFile.Length > 0)
                {
                    ...
                    // 파일명을 변경
                    // Guid.NewGuid() = 랜덤아이디생성
                    // Path.GetExtension() = 파일의 확장자만 가져옴 .jpg
                    string newFileName = Guid.NewGuid() + Path.GetExtension(UploadFile.FileName); // 65c29ddb-8de4-48c9-a4b2-e671ffaa8631.jpg
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (NewFile != null && NewFile.Length > 0)
                    {
                        ...
                        // Guid.NewGuid() = 랜덤아이디생성
                        // Path.GetExtension() = 파일의 확장자만 가져옴 .jpg
                        string newFileName = Guid.NewGuid() + Path.GetExtension(NewFile.FileName); // 65c29ddb-8de4-48c9-a4b2-e671ffaa8631.jpg
                        string filePath = Path.Combine(upFolder, newFileName);
